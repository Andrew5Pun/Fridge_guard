...
#
# Modified by Andrew Pun to include curl API communication
#         andrewpun.com
    ...
    then
        ...
        then
            ...
            then
                ...
                READING=$SIGN$INTEGER.$FRACTION

                EXECU="curl -u  "$NAME":"$PASS" -i -H \"Content-Type: application/json\" -X POST -d '{\"temperature\":\""$READING"\", \"serial\":\""$DEVICE"\", \"comment\":\"no comment\"}' [API address]"
